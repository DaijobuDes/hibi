...
Enable **MediaWiki** in **Settings → Addons** to open `.wiki` and `.mediawiki` documents with source highlighting, preview, and HTML export. Source files stay editable when disabled. Once enabled, its row in **Formats** opens this plugin's settings and native-tool check.
