...
the audio engine loads in the background after editing is ready. device discovery first prepares Chromium's audio service asynchronously so its cold startup does not block editing. this does not request microphone access, record audio, or retain device details. keyboard sounds become available when preparation and sample loading finish; typing never waits for them. disabling during preparation cancels engine startup.

only the chosen profile is fetched and decoded, locally; other profiles load on demand. buffers are cached while enabled. profile changes clear held keys, missing/not-yet-ready sounds are skipped, and stopping the addon removes listeners, toolbar actions, and the audio context. sounds already in progress stop on mute, blur, profile change, or disable. background test windows remain muted.
